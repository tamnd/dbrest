...
// ErrInvalidPreferences is PostgREST's PGRST122 (400): a request under
// Prefer: handling=strict that carries an unknown preference or a known one with
// a bad value. The message is upstream's; the offending tokens ride as details,
// comma-joined, so the caller sees exactly which preferences were rejected.
func ErrInvalidPreferences(offenders []string) *APIError {
	e := New(http.StatusBadRequest, CodeInvalidPreferences,
		"Invalid preferences given with handling=strict")
	return e.WithDetails("Invalid preferences: " + strings.Join(offenders, ", "))
}
